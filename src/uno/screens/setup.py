...
from textual.widgets import Header, Footer, Input, Label, Static, OptionList
...
class SetupScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="setup-outer"):
            with VerticalScroll(id="setup-layout"):
                ...
                yield Label("─── new game ───", id="setup-title")
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        if input_id == "starting-cards":
            self.query_one("#stacking-list", OptionList).focus()
            return

        if input_id and input_id.startswith("player-"):
